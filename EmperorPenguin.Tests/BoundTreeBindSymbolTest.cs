        ...
        if (cast<i64>(cls.fields.size()) == 1) {
            ...
            if (f is emperor.BoundDefinition.class_field) {
                if (f.class_field.field_symbol.is_some()) {
                    ...
                    if (sym.bound_type.kind is emperor.TypeKind.PrimitiveKind) {
    ...
    if (def is emperor.BoundDefinition.function_def) {
        ...
        if (func.scope.is_some()) {
            ...
            if (sym_a.is_some() && sym_b.is_some()) {
                if (sym_a.some is emperor.BoundSymbol.variable) {
                    let va = sym_a.some.variable;
                    println(""param_name="" + va.name);
                    println(""param_kind="" + cast<string>(va.variable_kind is emperor.VariableSymbolKind.Param));
...
initial {
    ...
    if (def is emperor.BoundDefinition.enum_def) {
        ...
        if (sym.is_some()) {
            if (sym.some is emperor.BoundSymbol.enum_member) {
                let es = sym.some.enum_member;
                if (es.bound_type.kind is emperor.TypeKind.EnumKind) {
