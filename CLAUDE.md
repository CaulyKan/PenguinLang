...
**The compiler only EMITS LLVM IR** (`.ll` + side files). Linking is external — `EmperorPenguin/emperor_penguin` (bash) / `emperor_penguin.bat` (Windows) check the LLVM environment, build the C runtime (`make -C EmperorPenguin/std/c`) and drive clang (native, cross linux→win, exe or `.penguin-lib`). `make release` deploys an emitter binary + driver script pair per platform; the emitted `.ll` is platform-independent (one emission, many links).
...
make bootstrap      # self-bootstrap EmperorPenguin -> build/bootstrap/pass2..pass4 (+ std dynlib + pass5 md5 convergence; host only)
make release        # release the host platform's emitter + driver script (release_linux / release_win)
make release_linux  # -> build/linux/emperor_penguin_llvm_emitter (thin exe), build/linux/libemperorpenguin.penguin-lib, build/linux/libemperorpenguin-std.penguin-lib, build/linux/emperor_penguin (+ EmperorPenguin/scripts/*.sh)
make release_win    # -> build/win/emperor_penguin_llvm_emitter.exe, build/win/emperor_penguin.bat (+ EmperorPenguin/scripts/*.bat; linux host cross-compiles via emperor)
make release_babypenguin        # self-contained single-file BabyPenguin publish (host) -> build/<host>/baby_penguin[.exe]
make release_babypenguin_linux  # -> build/linux/baby_penguin (Builtin.penguin/Utils.penguin copied beside it)
make release_babypenguin_win    # -> build/win/baby_penguin.exe (linux host cross-publishes via dotnet -r win-x64)
...
make publish        # deploy release+LSP+tools into the vscode extension (emitter+script+stdlib trees), baby_penguin publishes, vsix
...
- CI (`.github/workflows/dotnet.yml`): the `build` job runs bootstrap → lsp/tools → unittest → markdown suite; after the full pipeline passes (pushes/dispatches only, never PRs) it installs llvm-mingw (pinned 20260616, LLVM 22.1.8) + the vscode extension's npm deps and runs `make publish`, zips `build/linux` → `penguinlang-linux-YYYYMMDD.zip` and `build/win` → `penguinlang-win-YYYYMMDD.zip` (date version, UTC) and uploads both to the GitHub release `vYYYYMMDD` (create-or-clobber). Without the uncommitted `thirdparty/mingw-w64-x86_64-llvm-libs` package CI's windows emitter links with no-op JIT stubs. The `deploy-pages` job publishes `build/book` at the site root, `build/book-zh` under `/zh/`, and the latest test report at `/test-report/`.
...
- **Bootstrap is manual**: Pass2/Pass3 require native binaries `build/bootstrap/pass2`/`build/bootstrap/pass3` (built by `make bootstrap`). The runner **never** bootstraps; if a required binary is missing it exits non-zero telling you to run `make bootstrap`. The EmperorPenguin backends only EMIT `.ll`; the runner links every emission through `EmperorPenguin/emperor_penguin link/link-lib`. Pass1 and BabyPenguin only need `dotnet`.
...
Four more project files shape the build:

- `EmperorPenguinPass2.penguins` (formerly `EmperorPenguinFull.penguins`) — the same compiler set plus the json-backed Dynlib, json/vector/hashmap/array stdlib and `_utils` (the bootstrap's pass2 monolith, the pass2→pass3 step, and the WINDOWS emitter — on linux it is no longer the released compiler).
- `EmperorPenguinStd.penguins` — the **std dyn-lib**: `src/utils.penguin` + `std/penguin/{metaconfig,json,vector,hashmap,array,argparse}.penguin` → `libemperorpenguin-std.penguin-lib`. utils rides along because argparse references `_utils.*` and core_builtin's `#specializing` blocks target `_utils.List`; it ships as verbatim source like the other `#template` files. `dynlib.penguin` is NOT in the std lib — its libmeta builder/injector operates on the compiler's bound tree (`BoundCompilationUnit`/`SemanticModel`), so it stays a compiler module in EmperorPenguinLib.penguins (the consumer side, `load_lib_chain`, is part of every compiler binary). Built once by pass3 (the last stage that compiles std sources directly); everything after (pass4/pass5, release, LSP, tools) consumes it via `--lib`/auto-std. The `#template`/`#fun` files ship as VERBATIM SOURCE; metaconfig ships as an export-marked symbol-table entry. Any project consuming this lib must NOT also list these files in its own sources (double definition).
- `EmperorPenguinLib.penguins` — Full **minus main.penguin minus most std modules** (they live in the std lib; the Lib build passes `--lib <dir>/libemperorpenguin-std.penguin-lib`, so its libmeta records `deps=[libemperorpenguin-std]`; `dynlib.penguin` stays, see above): the whole compiler as `libemperorpenguin.penguin-lib` (lib mode triggers on the `.penguin-lib` output name). Its metadata is the **emperor-libmeta v1 structured symbol table** (see `std/penguin/dynlib.penguin`): `export`-marked defs + their signature/field/impl closure as declaration entries, every global (with re-bound initializer text), all `impl X for Y` edges, and VERBATIM SOURCE for files containing template/meta constructs (`#template`/`#fun`/`#specializing`) so consumers can monomorphize NEW generic instances. Consumers declare-not-define the table's defs — `--libmeta=direct` (default) injects prebuilt bound defs (skeleton registration + signature backfill + the publisher's prebuilt vtables) between semantic passes 1 and 2; `--libmeta=text` materializes bodyless declaration text instead (the debug/parity path; both modes produce byte-identical `.ll`) — then call into the `.so` for method bodies and re-specialize templates locally. Deps resolve **by name** from the referencing lib's directory → the compiler exe dir → cwd; the loader dedups by lib NAME (the same lib reached via an explicit `--lib` path AND via dep resolution never double-injects).
- `EmperorPenguinExe.penguins` — just `main.penguin`, linked with `--lib <dir>/libemperorpenguin.penguin-lib` (the std lib resolves through the dep chain). The exe carries the C runtime + optional JIT (the lib's `_emperor_*`/`__builtin.*` refs bind from it via `-rdynamic`) and initializes the lib's globals (re-defined from the embedded source, interposing the `.so`'s copies through the GOT). `link_lib` stamps the lib's basename as SONAME and `link_exe` adds `-rpath,$ORIGIN`, so an exe + `.penguin-lib` pair is relocatable.

`make bootstrap` keeps pass3 as the Full monolith (the first dyn-lib-capable compiler — pass2 comes from the ANTLR-safe stub project and cannot build libs). pass3 then builds the std dyn-lib (`pass4.d/libemperorpenguin-std.penguin-lib`, a FIXED input for both convergence rounds — pass5.d gets a byte-identical copy), and pass4/pass5 as lib+exe pairs in `build/bootstrap/pass4.d`/`build/bootstrap/pass5.d` (`build/bootstrap/pass4` is a symlink; convergence checks BOTH the exe and lib md5s; the pass5.d artifacts are KEPT so a repeat bootstrap re-verifies without recompiling). Every Makefile-driven compile passes its semantic flags explicitly (`--disable-coroutine`/`--disable-std`/`--enable-coroutine` + `--lib`), never relying on the `--enable-*` defaults — those exist for plain user invocations: `--enable-coroutine` and `--enable-std` default ON (`--disable-*` opts out); `--enable-std` probes `libemperorpenguin-std.penguin-lib` beside the compiler exe and silently does nothing when absent (bootstrap passes, the BabyPenguin VM, Windows monoliths). Building the compiler lib requires a JIT-capable compiler (build it with `-enable-meta`) — the compiler sources engage the meta engine during their own compilation.
...
1. Parse command-line args (`CompilerConfig.parse`; `--help` prints a usage summary and exits 0) → check for `.penguins` project file or direct `.penguin` files; project files may carry `lib = [...]` + `flags = [...]` (pre-scan before the lib chain)
2. **Auto-std**: when `--enable-std` (default) + dyn-lib-capable, probe `<compiler exe dir>/libemperorpenguin-std.penguin-lib` and push it into `lib_paths` (silent skip when absent or already named by `--lib`)
3. Load the `--lib` chain (deps recursively, deps-first, deduped by lib NAME) → merged `SourceInput`s + libmeta docs
4. Auto-load `core_builtin.penguin` + `io.penguin` (+ `scheduler.penguin` under `--enable-coroutine`, the default) as sources
5. Build per-file `SourceInput` list; `EmperorPenguinCompiler.compile_sources(inputs)` → bound compilation result
6. Check for semantic errors → report with source locations
7. `LLVMCompiler.build_ll(result)` → write `.ll` beside `-o` (+ `.def` export table, `.libs` consumer-lib closure for exes; `.libmeta` for lib mode). **No linking in-process** — the `emperor_penguin` driver script owns the C runtime build + clang link
...
| `array.penguin`, `vector.penguin`, `hashmap.penguin`, `json.penguin`, `metaconfig.penguin`, `argparse.penguin` (+ `src/utils.penguin`) | The **std dyn-lib** `libemperorpenguin-std.penguin-lib` (built from `EmperorPenguinStd.penguins` by pass3, deployed beside the compiler). Released compilers (lib+exe pairs: bootstrap pass4+, `build/linux/emperor_penguin_llvm_emitter`) auto-load it via `--enable-std` (default on) — user programs get `std.Vector`/`std.Hashmap`/`std.Array`/json/argparse with no flags. The Pass1/Pass2/Pass3 backends have no std lib beside them: auto-std silently no-ops and tests pass the sources via `Compile.Args` (e.g. `std.Array<T,N>`) |
| `dynlib.penguin` | NOT in the std lib — a COMPILER module (stays in `EmperorPenguinLib.penguins`): the emperor-libmeta builder/serializer/injector (operates on `BoundCompilationUnit`/`SemanticModel`) and the consumer side (`read_meta`, `load_lib_chain`, `compiler_exe_dir`, `dynlib_available`). Lib-name dedup: the same lib reached via an explicit `--lib` path AND via by-name dep resolution loads once |
| `argparse.penguin` | Clap-style command-line parsing from FIELD ANNOTATIONS (ships in the std dyn-lib — released compilers get it via auto-std; Pass2/Pass3 tests pass it + `vector.penguin` via `Compile.Args`). `#arg(help, short, long, required, default_text)` / `#pos_arg(help)` written directly before a field generate `_parse_arg_<field>()`/`_parse_pos_<field>()` marker functions returning `std.ArgInfo` (a dual-unit `#class`); `std.parse_args<T>()` / `std.parse_args_after_subcommand<T>()` splice `#argparse_parse` — a deferred meta call that JIT-generates the whole parse loop from `t.fields()`+`t.methods()`. Supported field types: bool, integers, f32/f64, string, `mut std.Vector<scalar>` (repeated option / rest positionals); `--` stops options; `-h/--help` prints help + exit(0); parse errors → stderr + exit(2); `std.argv()` wraps the process args. See Tests/StdlibTest/Argparse*.md |
...
(`_utils` with `List<T>`/`Queue<T>` and the file I/O externs lives in `EmperorPenguin/src/utils.penguin` — it ships inside the std dyn-lib as verbatim source; compiled from source in every bootstrap stage before the std lib exists (pass1/pass2, the Full monolith).)
