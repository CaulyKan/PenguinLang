...
**引用类型**不自动生成 `ICopy<T>`。实现 `ICopy<T>` 的引用类型必须自带深复制逻辑——且必须**显式**写出 `fun copy(this) -> Self` 方法（写在类内或 impl 块中）。若类的字段（传递地）包含引用类型，却只写空的 `impl ICopy<Self>;` 而不提供 `copy`，则是**编译错误**（`E_INTERFACE_IMPL`）：没有用户提供的 `copy`，调用会静默退化为浅克隆，使原始对象与副本共享被引用的对象。所有字段均为值类型的类保持自动生成的字节复制。
