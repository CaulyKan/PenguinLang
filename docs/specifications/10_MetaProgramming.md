...
>
> **`#fun` calling user code — `--meta-src <file>` / `meta-sources=[...]`.** By default a `#fun` body only sees unit B's base sources (the compiler's own type layer) and the compilation's other `#fun`s. Pass files EXPLICITLY — `--meta-src path` (repeatable, like `--lib`) on the CLI, or a `meta-sources=["a.penguin", ...]` key in a `.penguins` project (same glob rules as `sources=`, resolved against the project dir) — and the meta engine compiles exactly those files into unit B, so a `#fun` can call ordinary user functions (e.g. running an assembler at compile time). What you list is what unit B gets: there are no marker comments and no content heuristics, so listed files must be meta-safe by construction (free of coroutine/topology constructs — `initial`/`construct`/`connect`/`async fun`/`wait`/`yield`/ports would trip unit B's coroutine gate). A listed file need not be one of the compilation's own sources. A `#fun` declared inside a namespace is synthesized wrapped in that namespace together with the file's `using`s, plus a bare-name top-level forwarder so the JIT lookup keeps working. Unit B inherits the host's `--enable-coroutine` (and the scheduler stdlib) and skips definition-position `#fun`-call splicing (no meta re-entry). Example-scale consumer: `Examples/tinyriscv` (`meta-sources=[sw/Asm.penguin, sw/Firmware.penguin, esl Bit.penguin]` — compile-time firmware assembly, generated decoder and bus decode).
