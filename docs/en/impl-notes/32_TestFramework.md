...
* **Linking** (`EmperorLink`, 1414): the Emperor backends only emit `.ll`; the runner links via the `emperor` script — `emperor link <out>.ll -o <out>` for exes (consumer-lib closure from the `<out>.libs` side file) and `emperor link-lib <base>.ll <base>.libmeta -o <out>.penguin-lib` for libraries.
* **Artifacts**: per-combo workdir `build/testruns/<ts>/<compiler>/<category>/<test>/` with `source.penguin`, `out.exe`/`combined.ll`, `compile.log`, `run.log`, `result.json`. Every spawned stage process gets `TMPDIR = workdir` so the emperor script's temp artifacts land in the combo dir.
...
* `make test` / `make baseline_test` — the markdown suite (`TEST_ARGS` passthrough); `make unittest` — `dotnet test`; `make publish` — both platforms + vscode extension packaging + smoke tests (bundled emperor compile+link+run, bundled LSP session, wine-run Windows LSP); `make docs-site` — this site; `make gc-bench` — GC benchmarks.

### The emperor driver script (`EmperorPenguin/emperor`, 428 lines bash)

The compiler only emits IR; this script owns the LLVM toolchain work:

* `emperor [--emitter <path>] <src...> [flags] -o <out>` — full pipeline (compile → build C runtime via `make -C EmperorPenguin/std/c OUTPUT_DIR=<tmp>` → clang link). `-o *.penguin-lib` triggers lib mode; `--lib <x.penguin-lib>` both forwards to the emitter and links the consumer lib.
* `emperor link <file.ll> -o <out> [-enable-meta] [-target=win64] [--consumer-lib <so>]...`
* `emperor link-lib <file.ll> <file.libmeta> -o <out.penguin-lib>` — links the `.so`, stamps SONAME = output basename, appends metadata + `PENGUINLIB:<offset>:<size>` footer.
