...
The register-based IR is the shared middle layer of the PenguinLang compilers. Both implementations — BabyPenguin (C#) and EmperorPenguin (self-hosted PenguinLang) — lower their semantic trees into the **same IR design**: the same instruction set, the same value model (named registers `%x`, temporaries `%tN`, labels `name:`, globals `@name`), and byte-identical `display()` text. The IR is what lets the two compilers be swapped against each other in the cross-compiler test matrix.

What is shared is the **format**, not the code: each compiler re-implements the IR in its own language, and there is no IR-file interchange between them (the printers are debug output only).

| | BabyPenguin | EmperorPenguin |
|---|---|---|
| IR definition | `BabyPenguin/VirtualMachine/IRInstruction.cs` (597 lines) | `EmperorPenguin/src/ir/IRInstruction.penguin` (786 lines) |
| Producer | `BabyPenguin/VirtualMachine/IRGenerator.cs` — lowers the symbol-level BabyPenguinIR of each `ICodeContainer` (its docstring: "Translates BabyPenguin's existing compiled IR into the new EmperorPenguin-compatible IR format") | `EmperorPenguin/src/ir/IRGenerator.penguin` — lowers the Bound Tree |
| Consumers | the VM interpreter (`RuntimeFrame`) and the C# backend | `LLVMEmitter.lower()` → LLVM IR text |
| Debug dump | `IRModule.Display()`, `BP_DUMP_IR=/tmp/bp_ir.txt` | `IRPrinter.print_module()` |

Known deltas between the two implementations:

* Indirect calls: BabyPenguin splits them into `IRCallFuncPtrInst` / `IRCallFuncPtrVoidInst`; EmperorPenguin has a single `IRCallIndirect`.
* Raw pointers: EmperorPenguin adds `ADDRESS_OF` / `LOAD_PTR` / `STORE_PTR` (meta intrinsics `#__address_of`/`#__load`/`#__store`) with no BabyPenguin counterpart.
* Coroutine status: BabyPenguin's `IRRetInst` / `IRRetVoidInst` carry a `ReturnStatus` int (Blocked/YieldNotFinished/Finished/YieldFinished) for VM coroutine suspension; EmperorPenguin's `RET` has no status field.
* `CALL_VIRT` / `BOX` / `UNBOX` exist on both sides but are not implemented in the BabyPenguin VM interpreter (interface dispatch goes through funptr `RDMBR` + `CALL_FUNC_PTR` instead).

The rest of this page documents the IR itself, using the EmperorPenguin implementation as the reference.

**EmperorPenguin source file location**: `EmperorPenguin/src/ir/`
...
1. **High abstraction level**: hides memory layout details — no alloca/load/store/GEP
2. **Readability first**: every instruction has a `display()` method that generates human-friendly text
3. **Symbolic variables**: symbolic names (`%x`, `%this`) are used instead of numbered registers
4. **Source location tracking**: every instruction carries a `SourceLocation`, enabling error localization
5. **Implicit memory**: field access goes through RDMBR/WRMBR; the compiler handles the allocation strategy automatically

---

## IRSourceLocation

`IRSourceLocation.penguin` only keeps an empty namespace shell; source locations uniformly use `ast.SourceLocation` (the IR layer references it directly as `SourceLocation`).
...
Operands and results of IR instructions are uniformly represented as the `IRValue` enum:

| Variant | Class name | Fields | display format |
|------|------|------|-------------|
...
## Instruction Set (29 Core Instructions)

The `IRInstruction` enum contains 29 variants, divided into 6 functional categories:

### 1. Constants and Assignment (4)
...
Calls through a function-pointer value (`fun`-typed field / local variable); `callee` is the IRValue holding the fat function pointer.
...
### 6. Object Creation and Type Operations (8)
...
Used for pattern matching: checks whether an enum value is the specified variant.
...
Performs a runtime type check through the object's metadata: when `type_id` is an interface it looks up the interface_map; when it is a class name it compares metadata->name.
...
Heap-allocates and copies a value type (class or enum) for use as an interface reference (the value type → interface boxing direction is a copy).
...
Retrieves a value type back from a heap pointer. UNBOX is an aliasing view (it reuses the BOX's own storage), allowing `mut this` interface methods to write through to the object visible to the caller.
...
An IR function, corresponding to one function or the initial block in the Bound Tree.
...
The instruction emitter, encapsulating the instruction-building operations of an IRFunction. Each emit method automatically allocates a temporary register and adds the instruction.

### emit Methods

| Method | Signature | Generated instruction |
|------|------|-----------|
...
The Bound Tree → IR converter. It walks the definitions, expressions, and statements of the Bound Tree and generates the corresponding IR instruction sequences.
...
**SymbolRegEntry** — maps a symbol to an IR register:
```
SymbolRegEntry
...
**LoopLabels** — the header and exit labels of a loop:
```
LoopLabels
...
| Method | BoundExpression variant | Description |
|------|---------------------|------|
...
| Method | BoundStatement variant | Description |
|------|---------------------|------|
...
Outputs an IR module as human-readable text.
...
The IR module, containing all functions and global variables produced by a single compilation.
...
IR is the starting point of the compiler backend. It takes the Bound Tree (the intermediate representation carrying full semantic information) and generates an abstract, platform-independent instruction sequence. `src/llvm/LLVMCompiler.penguin` chains the two backend stages together: `build_ll()` first generates the IRModule with `IRGenerator.generate()`, then `LLVMEmitter.lower()` lowers it into LLVM IR text and writes the `.ll` file. The `.ll` is platform-independent; linking (clang + the C runtime library) is driven by the `emperor`/`emperor_penguin.bat` driver scripts outside the compiler.
