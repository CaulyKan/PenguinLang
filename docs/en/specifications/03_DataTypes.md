...
PenguinLang is statically typed. Explicit type annotations may be omitted when the compiler can infer the type from an initializer or context.

This chapter defines the primitive types, the value/reference classification, mutability, and assignment semantics. Classes, enums, and interfaces have their own chapters; generics are declared with `#template` (see [Class](./05_Class.md) and [Meta Programming](./11_MetaProgramming.md)).
...
`string` is an **immutable reference type**: a string value is a pointer to a header-prefixed block (`metaptr + length + data`, see *EmperorPenguin LLVM*), contents can never be modified after creation (every builtin producing a string allocates a fresh one), and assignment/argument passing **shares the pointer** (safe precisely because contents are immutable). Compare with `==` (content equality); build with `StringBuilder`.
...
All operations are index/unit-based (byte units on the native runtime, UTF-16 units on the BabyPenguin VM — identical for ASCII) and the case/trim tables are ASCII. Method calls dispatch directly — do not use `cast<IStringOps>`/`is IStringOps` on a string (interface-typed boxing of primitives is not supported).
...
For **reference types**, `ICopy<T>` is not auto-generated. A reference type that implements `ICopy<T>` must provide its own deep-copy logic — explicitly, as a `fun copy(this) -> Self` method (inside the class or the impl block). A bodyless `impl ICopy<Self>;` on a class whose fields (transitively) contain reference types is a **compile error** (`E_INTERFACE_IMPL`): without a user-provided `copy`, the call would silently lower to a shallow clone that shares the referenced objects between the original and the copy. Classes whose every field is value-like keep the auto-generated byte-copy.
...
Casting between class/interface types: object → implemented interface is implicit; interface → concrete class requires `cast` (a failed interface downcast raises a runtime error); value type → interface boxes (copies), interface → value type unboxes.
