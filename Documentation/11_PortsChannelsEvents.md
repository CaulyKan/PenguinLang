# Ports, Channels & Events

This chapter documents the RTL-style communication layer: module ports, `construct` wiring blocks, first-class channels, broadcast events, and the scheduling semantics underneath them (delta cycles, settle points, quiescence). The vocabulary is deliberately small:

- **`wait` is the only wakeup primitive.** It accepts a duration (`wait 5 tick`), a condition (`wait a == 5`), an edge (`wait change(x)` — parks until the watched value differs from its entry sample, yields the new value), an event (`let v : T = wait ev`), or a port/channel (`let v : T = wait this.x`). Every form that carries a payload returns the delivered value.
- **`event` / `emit` / `on` keywords are gone.** Broadcast is the first-class `Event<T>` value; subscription is a wait loop. There is no `poll`.
...
- Defaults and initial values (design Q3): a port's initial value is its explicit default **or the payload type's zero value**.
...
- A **bare port read** (`let v : i64 = s.out;`) is a **settle point** (design Q1): it first parks one delta at a time until the current simulation time's propagation has settled (the previous scheduler round carried no transaction activity and the current one is quiet so far), then samples the channel's *current slot*. The canonical example — `x = 2; println(f2.y)` — prints the propagated `2`, not the stale slot. Reading a port inside a `construct` block is a compile error (a settle point is a suspension, and constructs must not wait). `wait port` consumes the next transaction; `wait <condition>` re-evaluates every scheduler round through the settle read (`wait s.line == false` is the Verilog `wait()` idiom). `wait change(port)` is the one exception: it samples the **raw level** every round without settling — edge detection must see single-round pulses. A *plain variable* read keeps the next-tick semantics documented in `08_TimingModel.md`.
...
construct {
    ...
    connect(f1.y, q);          // output -> channel
    connect(q, f2.x);          // channel -> input
}
```
...
`connect(x, f.x)` where `x` is a `mut` variable turns the variable into a wire net — the design's canonical first example:
...
- Works for top-level construct `let`s (explicit `mut T` type required) and for class fields (`connect(this.baud, inner.clk)` — the UART shared-config-register shape); the net hub is a hidden field, per instance.
...
- Net reads stay plain variable reads (next-tick semantics; sample the current value anywhere the variable is visible). Fan-out works like an output port: each connect subscribes its own wire.
...
Two inversions define the territory of wire vs FIFO, confirmed by both the LSP-server and UART-RTL case studies:
...
- **Termination = all initial routines finished + scheduler quiescence.** Quiescence itself is not an exit — a server parked waiting for external input is a legal final state in the design — but v1 has no external event source, so a dead-still program simply ends. Use `exit()` for an explicit `$finish`.
