...
>
> **`#fun` calling user code — `--meta-with-user-sources`.** By default a `#fun` body only sees unit B's base sources (the compiler's own type layer) and the compilation's other `#fun`s. With the `--meta-with-user-sources` flag, the meta engine also compiles the compilation's own source files that OPT IN with a `// meta: unit-b` marker line (and are free of coroutine/topology constructs — `initial`/`construct`/`connect`/`async fun`/`wait`/`yield`/ports, checked on comment-stripped text) into unit B, so a `#fun` can call ordinary user functions (e.g. running an assembler at compile time). A `#fun` declared inside a namespace is synthesized wrapped in that namespace together with the file's `using`s, plus a bare-name top-level forwarder so the JIT lookup keeps working. Unit B inherits the host's `--enable-coroutine` (and the scheduler stdlib) and skips definition-position `#fun`-call splicing (no meta re-entry). Example-scale consumer: `Examples/tinyriscv` (compile-time firmware assembly, generated decoder and bus decode).
