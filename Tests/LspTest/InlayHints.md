# InlayHints
## Description
textDocument/inlayHint e2e against the prebuilt LSP server: the document's `let` declarations with inferred types get `: T` labels (kind 1) placed right after the variable name — `total: i64`, `name: string` at function top level and `inner: i64` inside the nested if block (the walker recurses through bound code blocks and if/while/lambda bodies). Types come from the bound variable symbols of the last error-free compile. Bodies byte-exact via the MATCH mode (Content-Length counts wildcarded as \d+). Server built by 'make lsp'.
